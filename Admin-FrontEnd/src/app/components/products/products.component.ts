...
exportToExcel() {
  const headers = Object.keys(this.products[0]).filter(key => key !== 'image').filter(key => key !== 'category');
  const data: any[][] = this.products.map(product => {
    const productWithoutImage = { ...product };
    delete (productWithoutImage as { image?: unknown }).image;
    return Object.values(productWithoutImage);
  });
